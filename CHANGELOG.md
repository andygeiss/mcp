...
- Bidi-test infrastructure hygiene sweep: the bidi happy-path tests (`Test_Server_With_SendRequest_Should_CorrelateResponse`, `Test_SendRequest_With_ElicitationCreate_AndCapabilityAdvertised_Should_RoundTrip`, `Test_ProgressReport_DuringOutboundBidi_Should_NotInterleave`) now bound their lifetime with `context.WithTimeout(t.Context(), 5*time.Second)`, drain the `srv.Run` goroutine via `t.Cleanup` so a `t.Fatalf` cannot leak it, check every `stdinW.Write` error, close `stdoutW` via cleanup so the reader cannot block on pipe-buffer drain, advertise `protocolVersion` in the initialize handshake, and assert clean stderr (no WARN/ERROR lines) on round-trip success. The two AI9 negative-path tests (`Test_Server_With_CapabilityGate_Should_RejectSamplingWithoutAdvertisement`, `Test_SendRequest_With_ElicitationCreate_AndCapabilityNotAdvertised_Should_ReturnCapabilityError`) now assert the exact total message count on stdout — a regression that emits a side effect via any method name fails the count, where the prior substring-only check would have missed it. (Story 3.2)
- `cmd/spec-coverage/main.go`: repurposed from a compile-checked stub into the fragment aggregator. Reads the per-package committed audit fragments under `docs/`, deduplicates rows by clause ID, sorts ascending, and writes the unified table to stdout. Cross-fragment duplicate clause IDs fail loudly (registry invariant violation). Stdlib-only. (Story 3.1)
- `docs/spec-coverage.protocol.txt` and `docs/spec-coverage.server.txt`: per-package audit fragments. The protocol fragment carries the 3 bootstrap clauses; the server fragment carries the 8 W2 clauses (Q5 + Q6 + Q18) that previously registered into the server test binary's `init()` blocks but were invisible to the renderer. The canonical `docs/spec-coverage.txt` is now the deterministic union of both, sorted by clause ID. (Story 3.1)
- `internal/server/spec_coverage_test.go`: per-package fragment renderer test mirroring the protocol-package convention. Tagged `//go:build integration` because Q5 clause registrations live in `internal/server/integration_test.go`; running under integration mode is the only way to see the full server registry. The Makefile target invokes it with `-tags=integration` automatically. (Story 3.1)
...
- `docs/agent-rules.md`: operational rules for AI agents — checked-in canonical source replacing the previously-gitignored project-context document. Downstream forks via `make init` now inherit the rules instead of getting broken citations to a file the scaffold doesn't ship.
