...
- [Tools Catalog](./TOOLS.md) — reflection-derived tool inventory; regenerated by `make catalog` (do not edit by hand)

## Conformance

- [Spec Conformance Narrative (draft)](./conformance/index.md) — Q2 draft wrapping `docs/spec-coverage.txt` for procurement / integrators (Q4 publication target)

## Guides

- [Scaffold Consumer Guide](./scaffold-consumer-guide.md) — orientation after `make init MODULE=...`: the welcome banner, the Edit/Wire/Verify flow, operator-dignity outcomes, what to read next
...
- [ADR-004](./adr/ADR-004-doctor-config-detection.md) — `mcp doctor` config-drift detection semantics, severity matrix, why no auto-remediation
